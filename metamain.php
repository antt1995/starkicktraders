...
if ($sched_counter > 3) {
	$sched_type=1;
	//$msg = "Scheduler failed to activate for $sched_counter minutes!\r\n";
  	//$hdrs = "From: SKT1 Scheduler Warning <nobody@berigames.com>\r\n";
  	//$e_response=mail("bengibbs@tmail.com","SKT1 Scheduler failure!",$msg,$hdrs);
...
$res1 = $db->Execute("SELECT * FROM $dbtables[browser] WHERE player_id=$playerinfo[player_id] AND browser= '$userAgent'");

if ($res1->RecordCount() == 0) {
	$res1 = $db->Execute("INSERT INTO $dbtables[browser] SET player_id=$playerinfo[player_id], browser='$userAgent'");
}
...
if($playerinfo[on_planet] == "Y")
{
  ...
  if($res2->RecordCount() != 0)
  {
    ...
    //echo "<META HTTP-EQUIV=\"Refresh\" CONTENT=\"0;URL=planet.php?kk=".date("U")."&planet_id=$playerinfo[planet_id]&id=".$playerinfo[player_id]."\">";
...
if ($browser != "hiptop")
	echo "<br><a href=report.php><img src=\"images/$shipinfo[image]\" border=0></a>".$towedimage;
?>
  </td>
 </tr>
</table>
...
if ($browser == "hiptop") {
?>
  <table width=100% border=0 cellspacing=25 cellpadding=5 align=center>
    <tr> 
      <td><a href="report.php"><?php echo "<img src=\"images/$shipinfo[image]\" border=0></a>$towedimage</td>"; ?>
      <td align=right> Turns available:<br>
        <b>
...
<tr><td  width=33%>
<font color=silver size=<? echo $basefontsize + 2; ?> face="arial">&nbsp;<? echo $l_turns_have; ?></font><BR><font color=white><b><? echo NUMBER($playerinfo[turns]) ?></b></font>
</td>
<td align=center  width=33%>
<font color=silver size=<? echo $basefontsize + 2; ?> face="arial"><? echo $l_turns_used ?></font><BR><font color=white><b><? echo NUMBER($playerinfo[turns_used]); ?></b></font>
</td>
<td align=right  width=33%>
<font color=silver size=<? echo $basefontsize + 2; ?> face="arial"><? echo $l_score?></font><BR><font color=white><b><? echo NUMBER($playerinfo[score])?>&nbsp;</b></font>
</td></tr>
</table>
<?php
}
...
<a href="<? echo "zoneinfo.php?kk=".date("U")."&zone=$zoneinfo[zone_id]"; ?>"><b><? echo "<font size=", $basefontsize + 2," face=\"arial\">$zoneinfo[zone_name]</font>"; ?></b></a>&nbsp;
</td></tr>
</table>
...
<td valign="top"><img src="images/lcorner.gif" width="8" height="11" border="0" alt=""></td>
...
else
{
  ...
  {
     echo "&nbsp;<a class=\"mnu\" href=\"move.php?kk=".date("U")."&sector=$links[$i]\">=&gt;&nbsp;$links[$i]</a>&nbsp;<a class=dis href=\"lrscan.php?kk=".date("U")."&sector=$links[$i]\">[$l_scan]</a>&nbsp;<br>";
...
echo "<div class=mnu>";
echo "&nbsp;<a class=dis href=\"lrscan.php?kk=".date("U")."&sector=*\">[$l_fullscan]</a>&nbsp;<br>";
?>
...
&nbsp;<a class=mnu href="mailto2.php?kk=<? echo date("U")?>"><? echo $l_send_msg ?></a>&nbsp;<br>
...
//if (!strstr($playerinfo[subscribed],"payment")) {
	echo "&nbsp;<a class=mnu href=subscribe.php?kk=".date("U")."><font color=red><b>Donate</b></font></a>&nbsp;<br>";
//}

?>
&nbsp;<a class=mnu href="help.php"><? echo $l_help ?></a>&nbsp;<br>
...
<tr><td nowrap>
&nbsp;<a class=mnu href="logout.php?kk=<? echo date("U") ?>"><? echo $l_logout ?></a>&nbsp;<br>
</td></tr>
</table>
...
if($num_planets > 0)
{
  ...
  while($i < $num_planets)
  {
    ...
    echo "<img src=\"images/$planettypes[$planetlevel]\" border=0><BR><font size=", $basefontsize + 1, " color=#ffffff face=\"arial\">";
    ...
    while(!$result4->EOF)
    {
		...
		if ($targetship[player_id]==0) {
			...
			if($roll < $success)
			{
				...
				$medalList = "";
				if (!$medRes->EOF) {
					$medalList = "&nbsp;<a href=ranking.php?detail=".urlencode($targetship[character_name]).">";
					while (!$medRes->EOF) {
						$medal = $medRes->fields;
				...
				if ($targetship[team_name]) {
					echo "<a href=ship.php?kk=".date("U")."&ship_id=$targetship[ship_id]><img src=\"images/", $targetship[image],"\" border=0></a><BR><font size=", $basefontsize +1, " color=#ffffff face=\"arial\">$targetship[ship_name]<br>($targetship[character_name])$medalList<br>(<font color=#33ff00>$targetship[team_name]</font>)</font>";
				}
				else
				{
...
if($num_defences > 0)
{
  ...
  while($i < $num_defences)
  {
    ...
    echo "$char_name ( ".NUMBER($qty)." $def_type )";
    echo "</font><BR>";
...
<br>
    ...
    <td valign=top> 
      ...
      <table border=1 cellpadding=1 cellspacing=0 bgcolor="#005000" align="center">
        <tr>
          <td NOWRAP> 
            <div class=mnu> &nbsp;<a class=mnu href="rsmove.php?kk=<? echo date("U")?>&engage=1&amp;destination=<? echo $playerinfo[preset1]; ?>">=&gt;&nbsp;
              <? echo $playerinfo[preset1]; ?>
              </a>&nbsp;<a class=dis href=preset.php>[
              <? echo $l_set?>
              ]</a>&nbsp;<br>
              &nbsp;<a class=mnu href="rsmove.php?kk=<? echo date("U")?>&engage=1&amp;destination=<? echo $playerinfo[preset2]; ?>">=&gt;&nbsp;
              <? echo $playerinfo[preset2]; ?>
              </a>&nbsp;<a class=dis href=preset.php>[
              <? echo $l_set?>
              ]</a>&nbsp;<br>
              &nbsp;<a class=mnu href="rsmove.php?kk=<? echo date("U")?>&engage=1&amp;destination=<? echo $playerinfo[preset3]; ?>">=&gt;&nbsp;
              <? echo $playerinfo[preset3]; ?>
              </a>&nbsp;<a class=dis href=preset.php>[
              <? echo $l_set?>
              ]</a>&nbsp;<br>
              &nbsp;<a class=mnu href="rsmove.php?kk=<? echo date("U")?>&engage=1&amp;destination=<? echo $playerinfo[preset4]; ?>">=&gt;&nbsp;
              <? echo $playerinfo[preset4]; ?>
              </a>&nbsp;<a class=dis href=preset.php>[
              <? echo $l_set?>
              ]</a>&nbsp;<br>
              &nbsp;<a class=mnu href="rsmove.php?kk=<? echo date("U")?>&engage=1&amp;destination=<? echo $playerinfo[preset5]; ?>">=&gt;&nbsp;
              <? echo $playerinfo[preset5]; ?>
              </a>&nbsp;<a class=dis href=preset.php>[
              <? echo $l_set?>
              ]</a>&nbsp;<br>
              &nbsp;<a class=mnu href="rsmove.php?kk=<? echo date("U")?>&engage=1&amp;destination=<? echo $playerinfo[preset6]; ?>">=&gt;&nbsp;
              <? echo $playerinfo[preset6]; ?>
              </a>&nbsp;<a class=dis href=preset.php>[
              <? echo $l_set?>
              ]</a>&nbsp;<br>
              &nbsp;<a class=mnu href="rsmove.php?kk=<? echo date("U")?>">=&gt;&nbsp;
              <? echo $l_main_other;?>
              </a>&nbsp;<br>
            </div>
          </td>
...
<td align="right"><img src="images/rcorner.gif" width="8" height="11" border="0" alt=""></td>
</tr></table>
        ...
        else
        {
          ...
          if($planet[name] == "")
            echo "$l_unnamed ";
		  	...
		  	if ($browser=="hiptop") {
            	echo substr($planet[name],0,5)."..<br>";
			} else {
				echo $planet[name]." ";
			}
		  }
