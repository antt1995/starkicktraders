...
if (!$result2 || $result2->RecordCount() == 0) {
	...
	if(!result3 || $result3->RecordCount() == 0) {
		echo "The IGB does not look kindly on hackers! (You are not in a sector with an IGB terminal any more)<br>";
		die();
	}
}
...
<!--
	...
	textarea.term {background-color: #000000; color: #00FF00; font-family:Courier New; font-size:10pt; border-color:#00FF00;}
-->
</STYLE>
...
elseif($command == 'consolidate3' && $limit_IGB==FALSE) //consolidate operation
  IGB_consolidate3();
elseif($command == 'view' && $playerinfo[player_id] == 1) // View statements
  IGB_statements();
elseif($command == 'view2' && $playerinfo[player_id] == 1) // View statements
  IGB_statements2($player_id);
elseif($command == 'viewstate')
  IGB_statements2($playerinfo[player_id]);
else
{
  echo "
  ...
  <td width=25% valign=bottom align=right><font size=2 color=#00FF00 face=\"courier new\"><a href=\"IGB.php?kk=".date("U")."&command=login\">$l_igb_login</a></td>
  ";
}
...
function IGB_login()
{
  ...
  echo "<tr><td colspan=2 align=center valign=top><font size=2 face=\"courier new\" color=#00FF00>$l_igb_welcometoigb<br>---------------------------------</td></tr>" .
       ...
       "</tr>" .
       "<tr><td colspan=2 align=center><font size=2 face=\"courier new\" color=#00FF00>$l_igb_operations<br>---------------------------------<br><br><a href=\"IGB.php?kk=".date("U")."&command=withdraw\">$l_igb_withdraw</a><br><a href=\"IGB.php?kk=".date("U")."&command=deposit\">$l_igb_deposit</a>";
  if ($limit_IGB==FALSE) {
	echo "<br><a href=\"IGB.php?kk=".date("U")."&command=transfer\">$l_igb_transfer</a><br><a href=\"IGB.php?kk=".date("U")."&command=loans\">$l_igb_loans</a>";
  }
  if ($playerinfo[player_id] == 1) {
	echo "<br><a href=\"IGB.php?kk=".date("U")."&command=view\">View Statements</a>";
  } else {
  	echo "<br><a href=\"IGB.php?kk=".date("U")."&command=viewstate\">View My Statement (Beta)</a>";
  }
  echo "<br>&nbsp;</td></tr>" .
       "<tr valign=bottom>" .
       "<td><font size=2 face=\"courier new\" color=#00FF00><a href=IGB.php>$l_igb_back</a></td><td align=right><font size=2 face=\"courier new\" color=#00FF00>&nbsp;<br><a href=\"main.php?kk=".date("U")."\">$l_igb_logout</a></td>" .
       "</tr>";
}
...
function IGB_withdraw()
{
  ...
  echo "<tr><td colspan=2 align=center valign=top><font size=2 face=\"courier new\" color=#00FF00>$l_igb_withdrawfunds<br>---------------------------------</td></tr>" .
       ...
       "<form action=IGB.php?kk=".date("U")."&command=withdraw2 method=POST>" .
...
function IGB_deposit()
{
  ...
  echo "<tr><td colspan=2 align=center valign=top><font size=2 face=\"courier new\" color=#00FF00>$l_igb_depositfunds<br>---------------------------------</td></tr>" .
       ...
       "<form action=IGB.php?kk=".date("U")."&command=deposit2 method=POST>" .
       "<input class=term type=text size=15 maxlength=20 name=amount value=0>" .
...
function IGB_statements()
{
  global $playerinfo;
  global $account;
  global $db, $dbtables;
  global $l_igb_back, $l_igb_logout;
  //echo "SELECT DISTINCT character_name, $dbtables[ibank_statement].player_id FROM $dbtables[players],$dbtables[ibank_statement] WHERE $dbtables[ibank_statement].player_id > 0 AND email NOT LIKE '%@furangee' AND $dbtables[players].player_id = $dbtables[ibank_statement].player_id ORDER BY character_name ASC<br>";
  
  $res = $db->Execute("SELECT DISTINCT character_name, $dbtables[ibank_statement].player_id FROM $dbtables[players],$dbtables[ibank_statement] WHERE $dbtables[ibank_statement].player_id > 0 AND email NOT LIKE '%@furangee' AND $dbtables[players].player_id = $dbtables[ibank_statement].player_id ORDER BY character_name ASC");
  while(!$res->EOF)
  {
    $ships[]=$res->fields;
    $res->MoveNext();
  }
  echo "<tr><td colspan=2 align=center valign=top><font size=2 face=\"courier new\" color=#00FF00>View Bank Statements<br>---------------------------------</td></tr>" .
       "<tr valign=top>" .
       "<form action=IGB.php?kk=".date("U")."&command=view2 method=POST>" .
       "<td><font size=2 face=\"courier new\" color=#00FF00>Choose player :<br><br>" .
  ...
  echo "</select></td><td valign=center align=right>" .
       "<input class=term type=submit name=shipt value=\" View statement \">" .
       "</form>" .
       "</td></tr>";
// ---- End statement form ---
...
function IGB_statements2($player_id)
{
  global $playerinfo;
  global $account;
  global $db, $dbtables;
  global $l_igb_back, $l_igb_logout;
  //global $player_id;
  echo "<tr><td colspan=2 align=center valign=top><font size=2 face=\"courier new\" color=#00FF00>View Bank Statements<br>---------------------------------<br>";
  $res = $db->Execute("SELECT character_name, amount, trans_type, trans_time FROM $dbtables[players],$dbtables[ibank_statement] WHERE $dbtables[players].player_id = $dbtables[ibank_statement].player_id AND $dbtables[ibank_statement].player_id = '$player_id' ORDER BY trans_time ASC");
  $statement = "";
  while (!$res->EOF) {
  	$transinfo=$res->fields;
	$year = substr($transinfo[trans_time], 0, 4);
    $month = substr($transinfo[trans_time], 4, 2);
    $day = substr($transinfo[trans_time], 6, 2);
	$hour = substr($transinfo[trans_time], 8, 2);
	$minute = substr($transinfo[trans_time], 10, 2);
	$second = substr($transinfo[trans_time], 12, 2);
    $date = date('M d, Y, H:i:s', mktime($hour, $minute, $second, $month, $day, $year));
	//echo "<tr><td align=left valign=top><font size=2 face=\"courier new\" color=#00FF00>";
	$transtype= array ("Unknown","Withdrew","Deposited","Borrowed","Repayed","Consolidated","Sent","Received","IGB Balance");
	$statement = $statement.$date." ".$transtype[$transinfo[trans_type]]." ".NUMBER($transinfo[amount])."\n";
	//echo $date." ".$transtype[$transinfo[trans_type]]." </td><td align=right valign=top><font size=2 face=\"courier new\" color=#00FF00>".NUMBER($transinfo[amount]);
  $res->MoveNext();
  }
  	echo "<textarea name='textarea' cols='60' rows='15' class='term'>$statement</textarea>";
	echo "</td></tr>";
// ---- End statement form ---

  echo "<tr valign=bottom>";
  if ($playerinfo[player_id]==1) {
       echo "<td><font size=2 face=\"courier new\" color=#00FF00><a href=IGB.php?kk=".date("U")."&command=view>$l_igb_back</a></td>";
  } else {
  		echo "<td><font size=2 face=\"courier new\" color=#00FF00><a href=IGB.php?kk=".date("U")."&command=login>$l_igb_back</a></td>";  }   
  echo "<td align=right><font size=2 face=\"courier new\" color=#00FF00>&nbsp;<br><a href=\"main.php?kk=".date("U")."\">$l_igb_logout</a></td>" .
       "</tr>";
}
...
function IGB_transfer()
{
  ...
  $res = $db->Execute("SELECT character_name, player_id FROM $dbtables[players] WHERE player_id > 0 AND email NOT LIKE '%@furangee' ORDER BY character_name ASC");
  while(!$res->EOF)
  {
    $ships[]=$res->fields;
    $res->MoveNext();
  ...
  echo "<tr><td colspan=2 align=center valign=top><font size=2 face=\"courier new\" color=#00FF00>$l_igb_transfertype<br>---------------------------------</td></tr>" .
       "<tr valign=top>" .
       "<form action=IGB.php?kk=".date("U")."&command=transfer2 method=POST>" .
...
// ---- begin Consol Credits form    // ---- added by Torr 
  echo "<tr valign=top>" .
       ...
       "<form action=IGB.php?kk=".date("U")."&command=consolidate method=POST>" .
...
function IGB_transfer2()
{
  ...
  {
    $res = $db->Execute("SELECT * FROM $dbtables[players] WHERE player_id=$player_id AND $player_id>0 AND email NOT LIKE '%@furangee'");

    if($playerinfo[player_id] == $player_id)
      IGB_error($l_igb_sendyourself, "IGB.php?kk=".date("U")."&command=transfer");

    if(!$res || $res->EOF)
      IGB_error($l_igb_unknowntargetship, "IGB.php?kk=".date("U")."&command=transfer");
    ...
    if($target[turns_used] < $IGB_min_turns)
    {
      ...
      IGB_error($l_igb_min_turns, "IGB.php?kk=".date("U")."&command=transfer");
    }

    if($playerinfo[turns_used] < $IGB_min_turns)
    {
      $l_igb_min_turns2 = str_replace("[igb_min_turns]", $IGB_min_turns, $l_igb_min_turns2);
      IGB_error($l_igb_min_turns2, "IGB.php?kk=".date("U")."&command=transfer");
    }
    ...
    if($IGB_trate > 0)
    {
      ...
      if(!$res->EOF)
      {
        ...
        IGB_error($l_igb_mustwait, "IGB.php?kk=".date("U")."&command=transfer");
      }
    }
    ...
    echo "<tr valign=top><td><font size=2 face=\"courier new\" color=#00FF00>$l_igb_recipient :</td><td align=right><font size=2 face=\"courier new\" color=#00FF00>$target[character_name]&nbsp;&nbsp;</td></tr>" .
         "<form action=IGB.php?kk=".date("U")."&command=transfer3 method=POST>" .
         "<tr valign=top>" .
         ...
         "$l_igb_transferrate" .
         "<tr valign=bottom>" .
         "<td><font size=2 face=\"courier new\" color=#00FF00><a href=IGB.php?kk=".date("U")."&command=transfer>$l_igb_back</a></td><td align=right><font size=2 face=\"courier new\" color=#00FF00>&nbsp;<br><a href=\"main.php?kk=".date("U")."\">$l_igb_logout</a></td>" .
         "</tr>";
  }
  else
  {
    if($splanet_id == $dplanet_id)
      IGB_error($l_igb_errplanetsrcanddest, "IGB.php?kk=".date("U")."&command=transfer");
    ...
    if($dest[base] == 'N')
      IGB_error($l_igb_errnobase, "IGB.php?kk=".date("U")."&command=transfer");
...
function IGB_transfer3()
{
  ...
  if(isset($player_id)) //ship transfer
  {
    ...
    if($playerinfo[player_id] == $player_id)
      IGB_error($l_igb_errsendyourself, "IGB.php?kk=".date("U")."&command=transfer");
    ...
    if($target[turns_used] < $IGB_min_turns)
    {
      ...
      IGB_error($l_igb_min_turns3, "IGB.php?kk=".date("U")."&command=transfer");
    }

    if($playerinfo[turns_used] < $IGB_min_turns)
    {
      $l_igb_min_turns4 = str_replace("[igb_min_turns]", $IGB_min_turns, $l_igb_min_turns4);
      IGB_error($l_igb_min_turns4, "IGB.php?kk=".date("U")."&command=transfer");
    }
    ...
    if($IGB_trate > 0)
    {
      ...
      if(!$res->EOF)
      {
        ...
        IGB_error($l_igb_mustwait2, "IGB.php?kk=".date("U")."&command=transfer");
      }
    }

    if(($amount * 1) != $amount)
      IGB_error($l_igb_invalidtransferinput, "IGB.php?kk=".date("U")."&command=transfer");

    if($amount == 0)
      IGB_error($l_igb_nozeroamount, "IGB.php?kk=".date("U")."&command=transfer");

    if($amount > $account[balance])
      IGB_error($l_igb_notenoughcredits, "IGB.php?kk=".date("U")."&command=transfer");
    ...
    if($IGB_svalue != 0)
    {
      ...
      if($amount > $maxtrans)
        IGB_error($l_igb_amounttoogreat, "IGB.php?kk=".date("U")."&command=transfer");
    }
    ...
    $db->Execute("UPDATE $dbtables[ibank_accounts] SET balance=balance-$amount,loantime=loantime WHERE player_id=$playerinfo[player_id]");
    $db->Execute("UPDATE $dbtables[ibank_accounts] SET balance=balance+$transfer,loantime=loantime WHERE player_id=$target[player_id]");
	...
	$db->Execute("INSERT INTO $dbtables[ibank_statement] SET player_id=$playerinfo[player_id], amount=$account[balance], trans_type=8"); // Type 8 = Balance update
    echo $db->ErrorMsg();
  }
  else
  {
    ...
    if($amount > $source[credits])
      IGB_error($l_igb_notenoughcredits2, "IGB.php?kk=".date("U")."&command=transfer");
...
function IGB_deposit2()
{
  ...
  $amount = StripNonNum($amount);
  if(($amount * 1) != $amount)
    IGB_error($l_igb_invaliddepositinput, "IGB.php?kk=".date("U")."&command=deposit");

  if($amount == 0)
    IGB_error($l_igb_nozeroamount2, "IGB.php?kk=".date("U")."&command=deposit");

  if($amount > $playerinfo[credits])
    IGB_error($l_igb_notenoughcredits, "IGB.php?kk=".date("U")."&command=deposit");
...
function IGB_withdraw2()
{
  ...
  $amount = StripNonNum($amount);
  if(($amount * 1) != $amount)
    IGB_error($l_igb_invalidwithdrawinput, "IGB.php?kk=".date("U")."&command=withdraw");

  if($amount == 0)
    IGB_error($l_igb_nozeroamount3, "IGB.php?kk=".date("U")."&command=withdraw");

  if($amount > $account[balance])
    IGB_error($l_igb_notenoughcredits, "IGB.php?kk=".date("U")."&command=withdraw");
  // Find out how many ships the customer has moored
  $res = $db->Execute("SELECT COUNT(*) as moored_ships FROM $dbtables[ships],$dbtables[universe] WHERE sector=sector_id AND port_type='special' AND on_planet='Y' AND player_id=$playerinfo[player_id]");
  $result = $res->fields;
  // Must keep 1000 credits per ship in the IGB if possible
...
function IGB_loans()
{
  ...
  if($account[loan] != 0)
  {
    ...
    echo "<form action=IGB.php?kk=".date("U")."&command=repay method=POST>" .
         "<tr valign=top>" .
  ...
  else
  {
    ...
    echo "<form action=IGB.php?kk=".date("U")."&command=borrow method=POST>" .
         "<tr valign=top>" .
...
function IGB_repay()
{
  ...
  if($account[loan] == 0)
    IGB_error($l_igb_notrepay, "IGB.php?kk=".date("U")."&command=loans");

  if($amount > $account[loan])
    $amount = $account[loan];

  if($amount > $playerinfo[credits])
    IGB_error($l_igb_notenoughrepay, "IGB.php?kk=".date("U")."&command=loans");
  ...
  $db->Execute("UPDATE $dbtables[ibank_accounts] SET loan=loan-$amount,loantime=loantime WHERE player_id=$playerinfo[player_id]");
  $db->Execute("UPDATE $dbtables[players] SET credits=credits-$amount WHERE player_id=$playerinfo[player_id]");
...
function IGB_consolidate()
{
  ...
  echo "<tr><td colspan=2 align=center valign=top><font size=2 face=\"courier new\" color=#00FF00>$l_igb_planetconsolidate<br>---------------------------------</td></tr>" .
       "<form action=IGB.php?kk=".date("U")."&command=consolidate2 method=POST>" .
       "<tr valign=top>" .
...
function IGB_consolidate2()
{
  ...
  echo "<tr><td colspan=2 align=center valign=top><font size=2 face=\"courier new\" color=#00FF00>$l_igb_planetconsolidate<br>---------------------------------</td></tr>" .
       ...
       "<form action=IGB.php?kk=".date("U")."&command=consolidate3 method=POST>" .
...
function IGB_error($errmsg, $backlink, $title="Error!")
{
  ...
  echo "<tr><td colspan=2 align=center valign=top><font size=2 face=\"courier new\" color=#00FF00>$title<br>---------------------------------</td></tr>" .
       ...
       "</tr>" .
       "<tr valign=bottom>" .
       "<td><font size=2 face=\"courier new\" color=#00FF00><a href=$backlink>$l_igb_back</a></td><td align=right><font size=2 face=\"courier new\" color=#00FF00>&nbsp;<br><a href=\"main.php?kk=".date("U")."\">$l_igb_logout</a></td>" .
       "</tr>" .
       "</table>" .
       "</td></tr>" .
       "</table>" .
