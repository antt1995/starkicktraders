...
else
{
  if(empty($module))
  {
    echo "Welcome to the StarKick Traders administration module<BR><BR>";
	...
	echo "<OPTION VALUE=massmail>Mass Mail</OPTION>";
  ...
  else
  {
    ...
    if($module == "useredit")
    {
      ...
      else
      {
        if(empty($operation))
        {
          ...
          $row = $res->fields;
		  $res = $db->Execute("SELECT * FROM $dbtables[ships] WHERE player_id=$user");
		  ...
		  while (!$res->EOF) {
			...
			echo "<TABLE BORDER=0 CELLSPACING=0 CELLPADDING=5>";
			  ...
			  echo "</TABLE></TD></TR>";
			  echo "<TR><TD>Current ship sector</TD><TD><INPUT TYPE=TEXT NAME=shipsector VALUE=\"$shipinfo[sector]\"></TD></TR>";
			  echo "</TABLE>";
          	  echo "<BR>";
          	  echo "<INPUT TYPE=HIDDEN NAME=user VALUE=$user>";
          	  echo "<INPUT TYPE=HIDDEN NAME=operation VALUE=save>";
			  echo "<INPUT TYPE=HIDDEN NAME=thisship VALUE=$shipinfo[ship_id]>";
			  echo "<INPUT TYPE=HIDDEN NAME=swordfish VALUE=$swordfish>";
			  echo "<INPUT TYPE=HIDDEN NAME=menu VALUE=useredit>";
          	  echo "<INPUT TYPE=SUBMIT VALUE=Save>";
			  echo "</FORM>";
        ...
        elseif($operation == "save")
        {
		  ...
		  echo "Ship Id = $thisship<br>";
		  if (empty($thisship)) {
          	$db->Execute("UPDATE $dbtables[players] SET character_name='$character_name',password='$password2',email='$email',turns='$turns',sector='$sector',credits='$credits',sector='$sector' WHERE player_id=$user");
		  } else {
		  echo "UPDATE $dbtables[ships] SET ship_name='$ship_name',ship_destroyed='$_ship_destroyed',hull='$hull',engines='$engines',power='$power',computer='$computer',sensors='$sensors',armour='$armour',shields='$shields',beams='$beams',torp_launchers='$torp_launchers',cloak='$cloak',dev_warpedit='$dev_warpedit',dev_genesis='$dev_genesis',dev_beacon='$dev_beacon',dev_emerwarp='$dev_emerwarp',dev_escapepod='$_dev_escapepod',dev_fuelscoop='$_dev_fuelscoop',dev_minedeflector='$dev_minedeflector',sector='$shipsector',ship_ore='$ship_ore',ship_organics='$ship_organics',ship_goods='$ship_goods',ship_energy='$ship_energy',ship_colonists='$ship_colonists',ship_fighters='$ship_fighters',torps='$torps',armour_pts='$armour_pts' WHERE player_id=$user AND ship_id=$thisship";
		  	$db->Execute("UPDATE $dbtables[ships] SET ship_name='$ship_name',ship_destroyed='$_ship_destroyed',hull='$hull',engines='$engines',power='$power',computer='$computer',sensors='$sensors',armour='$armour',shields='$shields',beams='$beams',torp_launchers='$torp_launchers',cloak='$cloak',dev_warpedit='$dev_warpedit',dev_genesis='$dev_genesis',dev_beacon='$dev_beacon',dev_emerwarp='$dev_emerwarp',dev_escapepod='$_dev_escapepod',dev_fuelscoop='$_dev_fuelscoop',dev_minedeflector='$dev_minedeflector',sector='$shipsector',ship_ore='$ship_ore',ship_organics='$ship_organics',ship_goods='$ship_goods',ship_energy='$ship_energy',ship_colonists='$ship_colonists',ship_fighters='$ship_fighters',torps='$torps',armour_pts='$armour_pts' WHERE player_id=$user AND ship_id=$thisship");
			}
    ...
    elseif($module == "massmail")
	{
		echo "<h1><font face=\"Verdana, Arial, Helvetica, sans-serif\">Mass Mail </font></h1>";
		echo "<p><font face=\"Verdana, Arial, Helvetica, sans-serif\">This sends mail to every ";
		echo "  registered player</font></p>";
		echo "<form name=\"form1\" method=\"post\" action=\"massmail.php\">";
		echo "<INPUT TYPE=HIDDEN NAME=swordfish VALUE=$swordfish>";
		echo "  <p> <font face=\"Verdana, Arial, Helvetica, sans-serif\">Subject: ";
		echo "    <input type=\"text\" name=\"subject\">";
		echo "    </font> </p>";
		echo "  <p><font face=\"Verdana, Arial, Helvetica, sans-serif\"> Message:</font></p>";
		echo "  <p><font face=\"Verdana, Arial, Helvetica, sans-serif\"> ";
		echo "    <textarea name=\"message\" rows= \"20\" cols=\"80\"></textarea>";
		echo "    </font></p>";
