...
else
{
   ...
   if(!isset($nummines) or !isset($numfighters) or !isset($mode))
   {
     ...
     $availfighters = NUMBER($shipinfo[ship_fighters]);
  ...
  else
  {
     ...
     {
      $l_mines_dmines=str_replace("[mines]",NUMBER($nummines), $l_mines_dmines);
        echo "$l_mines_dmines<BR>";
     }
     if ($numfighters > $shipinfo[ship_fighters])
     {
     ...
     {
     $l_mines_dfighter=str_replace("[fighters]",NUMBER($numfighters), $l_mines_dfighter);
	 ...
	 $update = $db->Execute("UPDATE $dbtables[ships] SET ship_fighters=ship_fighters-$numfighters,torps=torps-$nummines WHERE player_id=$playerinfo[player_id] AND ship_id=$playerinfo[currentship]");
