...
while (!$sql->EOF)
  {
   $row = $sql->fields;
   if ($row[amount] >= 10000000000) {
   		$sql2 = $db->Execute("select * from $dbtables[news] where user_id='$row[owner]' and news_type='col10000'");
   		if ($sql2->EOF) {
   			$colcount = 10000;
   			$name = get_player_name($row[owner]);
            $l_news_p_headline2=str_replace("[player]",$name,$l_news_p_headline);
   			$headline = $l_news_p_headline2 . $colcount . $l_news_cols;
   			$l_news_c_text100002=addslashes(str_replace("[name]",$name,$l_news_c_text10000));
   			$news = $db->Execute("INSERT INTO $dbtables[news] (headline, newstext, user_id, date, news_type) VALUES ('$headline','$l_news_c_text100002','$row[owner]',NOW(), 'col10000')");
   		}
  	}
   elseif ($row[amount] >= 5000000000) {
   		$sql2 = $db->Execute("select * from $dbtables[news] where user_id='$row[owner]' and news_type='col5000'");
   		if ($sql2->EOF) {
   			$colcount = 5000;
   			$name = get_player_name($row[owner]);
            $l_news_p_headline2=str_replace("[player]",$name,$l_news_p_headline);
   			$headline = $l_news_p_headline2 . $colcount . $l_news_cols;
   			$l_news_c_text50002=addslashes(str_replace("[name]",$name,$l_news_c_text5000));
   			$news = $db->Execute("INSERT INTO $dbtables[news] (headline, newstext, user_id, date, news_type) VALUES ('$headline','$l_news_c_text50002','$row[owner]',NOW(), 'col5000')");
   		}
  	}
   elseif ($row[amount] >= 1000000000) {
