  ...
  else
  {
    ...
    elseif($module == "furangeeedit")
    {
      ...
      if(empty($user))
      {
        ...
        $res = $db->Execute("SELECT email,character_name,ship_destroyed,active,sector FROM $dbtables[players] JOIN $dbtables[furangee] JOIN $dbtables[ships] WHERE email=furangee_id AND $dbtables[ships].player_id=currentship AND player_id=$dbtables[ship].player_id ORDER BY sector");
        while(!$res->EOF)
        {
          $row=$res->fields;
      ...
      else
      {
        if(empty($operation))
        {
		  ...
		  $row = $res->fields;
		  $res2 = $db->Execute("SELECT * FROM $dbtables[ships] WHERE player_id=$row[player_id] AND player_id=$row[currentship]");
...
// *****************************************************************************
		$result2 = $db->Execute("INSERT INTO $dbtables[players] (`player_id`, `character_name`, `password`, `email`,  `credits`, `sector`, `turns`, `on_planet`, `turns_used`, `last_login`, `interface`, `ip_address`, `alerts`, `currentship`) VALUES ('', '$character', '$makepass', '$emailname', '$start_credits', '$sector', '$start_turns','N', '3000', '$stamp', 'N', '127.0.0.1','N','1')");
		// Calculate ship type
		$shipavg = $furlevel;
		$shipavg /= 7;
		if($shipavg < 8)
		...
		elseif($shipavg < 20)
		   ...
		   $shiplevel = 5;
		$result3 = $db->Execute("INSERT INTO $dbtables[ships] (`ship_id`, `player_id`, `type`, `ship_name`, `ship_destroyed`, `hull`, `engines`, `power`, `computer`, `sensors`, `beams`, `torp_launchers`, `torps`, `shields`, `armour`, `armour_pts`, `cloak`, `sector`, `ship_ore`, `ship_organics`, `ship_goods`, `ship_energy`, `ship_colonists`, `ship_fighters`, `ship_damage`, `on_planet`, `dev_warpedit`, `dev_genesis`, `dev_beacon`, `dev_emerwarp`, `dev_escapepod`, `dev_fuelscoop`, `dev_minedeflector`, `planet_id`, `cleared_defences`, `dev_lssd`) VALUES ('', LAST_INSERT_ID(), '$shiplevel', '$shipname', 'N', $furlevel,$furlevel,$furlevel,$furlevel,$furlevel,$furlevel,$furlevel,$maxtorps,$furlevel,$furlevel,$maxarmour,$furlevel, $sector,0,0,0,$maxenergy,0,$maxfighters, '', 'N', '0', '0', '0', '0', 'N', 'N', '0', '0', NULL, 'N')"); 
          //$result2 = $db->Execute("INSERT INTO $dbtables[players] VALUES('','$shipname','N','$character','$makepass','$emailname',$furlevel,$furlevel,$furlevel,$furlevel,$furlevel,$furlevel,$furlevel,$maxtorps,$furlevel,$furlevel,$maxarmour,$furlevel,$start_credits,$sector,0,0,0,$maxenergy,0,$maxfighters,$start_turns,'','N',0,0,0,0,'N','N',0,3000, '$stamp',0,0,0,0,'N','127.0.0.1',0,0,0,0,'Y','N','N','Y','','$default_lang','N','N','N','',0,0,0,0,1)");
          if(!$result2 | !result3) {
            echo $db->ErrorMsg() . "<br>";
          } else {
