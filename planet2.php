...
else
{
  ...
  if($allore==-1)
  {
     ...
     if ($tpore == -1)
     {
        $transfer_ore = $shipinfo['ship_ore'];
     }
     else
     {
  ...
  if($allorganics==-1)
  {
     if ($tporganics==-1)
     {
        $transfer_organics = $shipinfo['ship_organics'];
     }
     else
     {
  ...
  if($allgoods==-1)
  {
     if ($tpgoods==-1)
     {
        $transfer_goods = $shipinfo['ship_goods'];
     }
     else
     {
  ...
  if($allenergy==-1)
  {
     if ($tpenergy==-1)
     {
        $transfer_energy = $shipinfo['ship_energy'];
     }
     else
     {
  ...
  if($allcolonists==-1)
  {
     if ($tpcolonists==-1)
     {
        $transfer_colonists = $shipinfo['ship_colonists'];
     }
     else
     {
  ...
  if($allfighters==-1)
  {
     if ($tpfighters==-1)
     {
        $transfer_fighters = $shipinfo['ship_fighters'];
     }
     else
     {
  ...
  if(($tpore == -1) && ($transfer_ore > $shipinfo['ship_ore']))
  {
    $transfer_ore = $shipinfo['ship_ore'];
  ...
  if(($tporganics == -1) && ($transfer_organics > $shipinfo['ship_organics']))
  {
    $transfer_organics = $shipinfo['ship_organics'];
  ...
  if(($tpgoods == -1) && ($transfer_goods > $shipinfo['ship_goods']))
  {
    $transfer_goods = $shipinfo['ship_goods'];
  ...
  if(($tpenergy == -1) && ($transfer_energy > $shipinfo['ship_energy']))
  {
    $transfer_energy = $shipinfo['ship_energy'];
  ...
  if(($tpcolonists == -1) && ($transfer_colonists > $shipinfo['ship_colonists']))
  {
    $transfer_colonists = $shipinfo['ship_colonists'];
  ...
  if(($tpcredits == -1) && $planetinfo[base] == 'N' && ($transfer_credits + $planetinfo['credits'] > $max_credits_without_base))
  {
     ...
     echo "$l_planet2_baseexceeded $l_planet2_settr ".NUMBER($transfer_credits)." $l_credits.<BR>\n";
  }
  ...
  if(($tpfighters == -1) && ($transfer_fighters > $shipinfo['ship_fighters']))
  {
    $transfer_fighters = $shipinfo['ship_fighters'];
  ...
  else
  {
    if(!empty($planetinfo))
    {
      ...
      {
        if($transfer_ore < 0 && $shipinfo[ship_ore] < abs($transfer_ore))
        {
        ...
        if($transfer_organics < 0 && $shipinfo[ship_organics] < abs($transfer_organics))
        {
        ...
        if($transfer_goods < 0 && $shipinfo[ship_goods] < abs($transfer_goods))
        {
        ...
        if($transfer_energy < 0 && $shipinfo[ship_energy] < abs($transfer_energy))
        {
        ...
        if($transfer_colonists < 0 && $shipinfo[ship_colonists] < abs($transfer_colonists))
        {
        ...
        if($transfer_fighters < 0 && $shipinfo[ship_fighters] < abs($transfer_fighters))
        {
